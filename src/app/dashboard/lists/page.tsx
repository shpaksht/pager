...
import { ReadingListsOverview } from "@/components/dashboard/ReadingListsOverview";
...
export default async function ListsPage() {
  ...
  const [lists, latestSpeedTest, chapterStats] = await Promise.all([
  ...
  const normalizedLists = lists.map((list) => {
    ...
    const readWords = items.reduce(
      (sum, item) => sum + Math.round((item.progressPercent / 100) * item.wordCount),
      0
    );
    const totalHours = items.reduce((sum, item) => sum + (item.requiredHours ?? 0), 0);
    const remainingHours = items.reduce((sum, item) => sum + (item.remainingHours ?? 0), 0);
    ...
    return {
      ...
      previewBooks: items.slice(0, 3).map((item) => ({
        id: item.id,
        title: item.title,
  ...
  return <ReadingListsOverview initialLists={normalizedLists} />;
}
